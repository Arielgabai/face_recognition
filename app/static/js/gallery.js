    ...
    async openLightbox(index) {
        ...
        const drawFaces = async () => {
          try {
            ...
            const res = await fetch(`/api/photo/${photoId}/faces`, {
                headers: token ? { 'Authorization': `Bearer ${token}` } : {}
            });
            if (!res.ok) return;
            const data = await res.json();
            const boxes = Array.isArray(data.boxes) ? data.boxes : [];
            // Créer un overlay container par dessus l'image
            let overlay = this.lightboxElement.querySelector('.gallery-face-overlays');
            if (!overlay) {
                overlay = document.createElement('div');
                overlay.className = 'gallery-face-overlays';
                ...
                overlay.style.maxHeight = '95%';
                this.lightboxElement.appendChild(overlay);
            } else {
                overlay.innerHTML = '';
            }

            // Adapter l'overlay à la taille affichée de l'image
            ...
            boxes.forEach((b) => {
                ...
                box.style.left = `${Math.max(0, Math.min(1, b.left || 0)) * displayedWidth}px`;
                box.style.top = `${Math.max(0, Math.min(1, b.top || 0)) * displayedHeight}px`;
                box.style.width = `${Math.max(0, Math.min(1, b.width || 0)) * displayedWidth}px`;
                box.style.height = `${Math.max(0, Math.min(1, b.height || 0)) * displayedHeight}px`;
                box.style.border = b.matched ? '3px solid #4caf50' : '2px solid rgba(255,255,255,0.9)';
                box.style.borderRadius = '6px';
                box.style.boxShadow = b.matched ? '0 0 12px rgba(76,175,80,0.8)' : '0 0 8px rgba(0,0,0,0.6)';
                box.style.zIndex = 3;
                ...
                if (b.matched || (typeof b.confidence === 'number')) {
                    ...
                    label.style.background = b.matched ? 'rgba(76,175,80,0.95)' : 'rgba(0,0,0,0.7)';
